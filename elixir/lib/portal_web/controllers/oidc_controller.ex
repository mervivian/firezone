defmodule PortalWeb.OIDCController do
  ...
  defmodule Database do
    ...
    @pending_identity_conflict_replace_fields [
      :actor_id,
      :updated_at | @pending_identity_profile_fields
    ]
    ...
    def fetch_active_actor_by_email(account, email) do
      ...
      from(actor in Portal.Actor,
        ...
        where: is_nil(actor.disabled_at),
        where: fragment("? = ?::citext", actor.email, ^email),
        preload: [:account],
        limit: 1
      )
      |> Safe.unscoped(:replica)
    ...
    defp insert_external_identity_from_pending(%PendingIdentity{} = pending_identity) do
      ...
      |> change(attrs)
      |> ExternalIdentity.changeset()
      |> then(&Safe.insert(Portal.Repo, &1, external_identity_upsert_opts()))
    end

    defp external_identity_upsert_opts do
      [
        on_conflict: {:replace, @pending_identity_conflict_replace_fields},
        conflict_target: [:account_id, :idp_id, :issuer],
        returning: true
      ]
    end
