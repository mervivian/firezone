defmodule PortalAPI.Client.Channel do
  ...
  @doc """
  `:on_failed` callback wired into the policy_authorization `Portal.Queue`.

  Invoked from the Queue's GenServer process when a buffered policy
  authorization fails to persist (FK violation, or any other Postgrex error
  on the batch). Delivers `:reject_access` to the receiving device (gateway
  or peer client in the c2c case) via `Portal.PG`. Because the matching
  `:allow_access` was also dispatched from the same Queue pid (via the
  `:dispatch` callback passed to `Queue.enqueue/3`), Erlang's per-pid signal
  ordering guarantees the receiver sees `:allow_access` before this
  `:reject_access`.

  The message carries a synthetic `%Portal.PolicyAuthorization{}` so the
  receiver can take the per-authz_id eviction path on its local cache —
  same shape as the CDC delete handler. If other authorizations still grant
  the same `(client, resource)` access, the receiver pushes
  `access_authorization_expiry_updated`; otherwise it pushes `reject_access`.
  """
  def revoke_receiver_access(attrs, _metadata) do
  ...
  @doc """
  `:on_confirmed` callback wired into the policy_authorization `Portal.Queue`.

  Invoked from the Queue's GenServer process for every entry that
  successfully persisted on a flush. Sends `:confirm_authz_durability` to the
  receiving device so the receiver can cancel its authz durability timer,
  proving the authorization is durably backed by a DB row and won't need
  to be revoked on queue death.

  Sent from the Queue's pid via PG, so it's ordered after the original
  allow message (same sender pid).
  """
  def confirm_authz_durability(attrs) do
    PG.deliver(
      attrs.receiving_device_id,
  ...
  def confirm_session_durability(attrs) do
