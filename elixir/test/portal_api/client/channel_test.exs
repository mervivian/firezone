defmodule PortalAPI.Client.ChannelTest do
  ...
  describe "handle_info/2 for presence events" do
    ...
    test "disconnected_ids only contains relays that are truly offline", %{
      ...
      # 2. Selecting the best relays to send to the client (connected)
      #
  ...
  describe "handle_info/2 flow_creation_timeout" do
    ...
    test "ignores late {:connect, ...} after flow has already timed out", %{
      dns_resource: resource,
      dns_resource_policy: policy,
      membership: membership,
      client: client,
  ...
  describe "handle_in/3 reuse_connection" do
    ...
    test "broadcasts allow_access to the gateways and then returns connect message", %{
      ...
      assert_receive {:allow_access, {channel_pid, socket_ref}, payload}

      assert %{
               resource: recv_resource,
               client_id: recv_client_id,
               client_ipv4: _client_ipv4,
               client_ipv6: _client_ipv6,
               authorization_expires_at: authorization_expires_at,
               client_payload: "DNS_Q"
             } = payload

      assert recv_resource.id == Ecto.UUID.dump!(resource_id)
      assert recv_client_id == client_id
      assert authorization_expires_at == socket.assigns.subject.expires_at
  ...
  describe "handle_in/3 request_connection" do
    ...
    test "broadcasts request_connection to the gateways and then returns connect message", %{
      ...
      assert recv_client.peer.preshared_key == "PSK"
      assert recv_client.payload == "RTC_SD"
      assert recv_resource.id == Ecto.UUID.dump!(resource_id)
  ...
  describe "confirm_authz_durability/1 (policy_authorization queue on_confirmed callback)" do
    test "delivers :confirm_authz_durability to the receiving device via PG carrying the authz_id" do
      ...
      assert :ok = PortalAPI.Client.Channel.confirm_authz_durability(attrs)
      assert_receive {:confirm_authz_durability, ^policy_authorization_id}
    end
  end

  describe "revoke_receiver_access/2 (policy_authorization queue on_failed callback)" do
    test "delivers reject_access to the receiving device via PG carrying a synthetic PolicyAuthorization" do
      receiving_device_id = Ecto.UUID.generate()
      initiating_device_id = Ecto.UUID.generate()
      resource_id = Ecto.UUID.generate()
      ...
      assert :ok = PortalAPI.Client.Channel.revoke_receiver_access(attrs, nil)

      assert_receive {:reject_access, %Portal.PolicyAuthorization{} = pa}
      assert pa.id == policy_authorization_id
      assert pa.account_id == account_id
      assert pa.initiating_device_id == initiating_device_id
      assert pa.receiving_device_id == receiving_device_id
      assert pa.resource_id == resource_id
      assert pa.token_id == token_id
      assert pa.expires_at == expires_at
    end

    test "returns :ok when the receiver is not registered in PG" do
      attrs = %{
        receiving_device_id: Ecto.UUID.generate(),
        initiating_device_id: Ecto.UUID.generate(),
        resource_id: Ecto.UUID.generate()
